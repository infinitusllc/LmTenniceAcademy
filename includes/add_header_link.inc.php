...
if (isset($_POST['submit'])) {
    ...
    if ($change == '0') { // if a new link is created
    ...
    } elseif ($change == '1') { // if an existing link is updated

        $ID = mysqli_real_escape_string($conn, $_POST["id"]);
        $sql = "UPDATE header_links SET url = '$url', keyword = '$keyword', weight = $weight WHERE id = $ID";
        if (!empty($parent_id))
            $sql = "UPDATE header_links SET url = '$url', keyword = '$keyword', weight = $weight, parent_id = $parent_id WHERE id = $ID";
        ...
        if (mysqli_query($conn, $sql)) {
            ...
            foreach ($languages as $language) { //insert into header_content corresponding name and description
                ...
                $sql = "UPDATE header_content SET `name` = '$name', description = '$desc' WHERE lang_key = $lang_key AND group_id = $ID";
                mysqli_query($conn, $sql);
            }
